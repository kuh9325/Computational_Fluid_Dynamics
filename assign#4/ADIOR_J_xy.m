...
function ADIOR_J_xy(tmp,beta,tol,x,y,k,imax,jmax,kmax)
        ...
        for j = 2:jmax-1
            if j == 2
                nKV(j-1) = -(1 -rp)*(2*(1 + beta^2))*mtmp(i,j)...
                    -rp*(mtmp(i+1,j) + mtmp(i-1,j)) -nb*mtmp(i,j-1);
            elseif j == jmax-1
                nKV(j-1) = -(1 -rp)*(2*(1 + beta^2))*mtmp(i,j)...
                    -rp*(mtmp(i+1,j) + mtmp(i-1,j)) -na*mtmp(i,j+1);
            else
                nKV(j-1) = -(1 -rp)*(2*(1 + beta^2))*mtmp(i,j)...
                    -rp*(mtmp(i+1,j) + mtmp(i-1,j));
            end
        end
        nUK = nKC\nKV;
        for j = 2:jmax-1
...
xlabel('x');
ylabel('y');
title('using ADIOR method: Jacobi (x -> y)');
