...
function PSOR_GS(tmp,beta,tol,x,y,k,imax,jmax,kmax)
    ...
    for j = 2:jmax-1
        for i = 2:imax-1
            ntmp(i,j) = (1-rp)*tmp(i,j) + (rp/(2*(1 + beta^2)))*(tmp(i+1,j)...
                + ntmp(i-1,j) + (beta^2)*(tmp(i,j+1) + ntmp(i,j-1)));
        end
    end
...
fprintf('\n@@ PSOR method: Gauss-Seidel @@\n')
fprintf('Number of iterations = \t %8.0f \nCalculation time =%12.0f sec \n',k,t)
Errtxt = fopen('PSOR:Gauss-Seidel.txt','w');
fprintf(Errtxt, '%d     %d \n', [iter; Err]);
...
xlabel('x');
ylabel('y');
title('using PSOR method: Gauss-Seidel');
