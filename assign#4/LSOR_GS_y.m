...
function LSOR_GS_y(tmp,beta,tol,x,y,k,imax,jmax,kmax)
...
for k = 1:kmax
    ...
    for i = 2:imax-1
        for j = 2:jmax-1
            if j == 2
                KV(j-1) = -2*(1 + beta^2)*(1-rp)*tmp(i,j) -rp*(tmp(i+1,j)...
                    + ntmp(i-1,j)) -b*ntmp(i,j-1);
            elseif j == jmax-1
                KV(j-1) = -2*(1 + beta^2)*(1-rp)*tmp(i,j) -rp*(tmp(i+1,j)...
                    + ntmp(i-1,j)) -a*ntmp(i,j+1);
            else
                KV(j-1) = -2*(1 + beta^2)*(1-rp)*tmp(i,j) -rp*(tmp(i+1,j)...
                    + ntmp(i-1,j));
            end
...
fprintf('\n@@ LSOR method: Gauss-Seidel (y sweep) @@\n')
fprintf('Number of iterations = \t %8.0f \nCalculation time =%12.0f sec \n',k,t)
Errtxt12 = fopen('LSOR:Gauss-Seidel(y).txt','w');
fprintf(Errtxt12, '%d     %d \n', [iter; Err]);

contour(x,y,T,'Showtext','on')
grid on
xlabel('x');
ylabel('y');
title('using LSOR method: Gauss-Seidel (y sweep)');
