...
function LSOR_GS_x(tmp,beta,tol,x,y,k,imax,jmax,kmax)
        ...
        for i = 2:imax-1
            if i == 2
                KV(i-1) = -2*(1 + beta^2)*(1-rp)*tmp(i,j)...
                    -rp*beta^2*(tmp(i,j+1) + ntmp(i,j-1)) -b*ntmp(i-1,j);
            elseif i == imax-1
                KV(i-1) = -2*(1 + beta^2)*(1-rp)*tmp(i,j)...
                    -rp*beta^2*(tmp(i,j+1) + ntmp(i,j-1)) -a*ntmp(i+1,j);
            else
                KV(i-1) = -2*(1 + beta^2)*(1-rp)*tmp(i,j)...
                    - rp*beta^2*(tmp(i,j+1) + ntmp(i,j-1));
            end
        end
        UK = KC\KV;
        for i = 2:imax-1
...
fprintf('\n@@ LSOR method: Gauss-Seidel (x sweep) @@\n')
fprintf('Number of iterations = \t %8.0f \nCalculation time =%12.0f sec \n',k,t)
Errtxt = fopen('LSOR:Gauss-Seidel(x).txt','w');
fprintf(Errtxt, '%d     %d \n', [iter; Err]);
...
xlabel('x');
ylabel('y');
title('using LSOR method: Gauss-Seidel (x sweep)');
